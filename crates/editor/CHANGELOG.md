...
- **Comment threads as file-tree entries**: Files in the PR sidebar with review comments now show an expansion chevron next to the comment count. Clicking the comment chip (or the chevron) expands the file row to reveal each thread as a navigable child entry — showing the author, a one-line snippet, the line number, and an unseen/resolved badge. Clicking a thread selects the file and animates the diff scroll to that line.
- **Thread resolution (GitHub)**: Review threads can now be resolved and unresolved without leaving the editor. Each thread card shows a Resolve/Unresolve button and, when resolved, a "Resolved" pill. State is fetched via GitHub's GraphQL API (`reviewThreads`) and synced with `resolveReviewThread` / `unresolveReviewThread` mutations; local state updates optimistically for snappy UI. A new `Shift+R` shortcut toggles resolution on the thread nearest the cursor, and a filter pill in the file panel header hides resolved threads from the tree.
...
- **Depth pass on floating surfaces**: Elevated surfaces that were previously flat-filled rectangles now read as floating via consistent soft drop shadows. Added a reusable `OverlayStyle::elevated_card` preset (offset 4, blur 14, alpha 55, corner 6) — a mid-tier between `frosted_glass` (full-modal) and the notification toast. Applied to the PR review Submit-Review panel and Merge popup (previously plain `egui::Frame::new()` with no shadow). The PR review floating comment cards now use a proper blurred shadow via `egui::epaint::Shadow::as_shape` plus a `bg_elevated` card fill, replacing the single hard-edged alpha-12 rect that provided minimal depth perception.
...
- **Diff readability pass**: Four changes that make long diffs easier to scan. (1) Context-line text is dimmed to ~62% opacity via the tree-sitter span pass, so additions and deletions visually leap off the page. (2) A sticky function-context header pins the current hunk's `@@ fn foo() @@` label to the top of the diff viewport whenever the real header scrolls out of view, in both unified and split modes. (3) Context lines now show thin indent guides under leading whitespace (1 rule per 4-space level) and a 2.5% zebra band every 5 lines, giving the eye structural rhythm without adding color noise. (4) Syntax spans on addition/deletion lines are now contrast-boosted against the diff background to at least WCAG AA (4.5:1). Without this, muted tokens like `//` comments on light themes (Parchment, Light, Stockholm, Copenhagen) sat at ~3.0:1 contrast on the green/red tint and were hard to read; a binary-search hue-preserving blend pushes them toward the nearest extreme only as far as needed, leaving already-vivid colors and well-contrasted dark-theme palettes untouched.
