-- Share to Slack/Discord Plugin
-- Allows sharing current pane context to Slack or Discord via webhooks.
--
-- Setup:
-- 1. Create a Slack incoming webhook: https://api.slack.com/messaging/webhooks
--    or Discord webhook: Server Settings > Integrations > Webhooks
...
-- 5. Type :share-discord [message] to share to Discord
-- 6. Type :share-clipboard to copy context to clipboard
--
-- Example:
--   :share-slack Check out this latency spike!
--   :share-discord Anyone seeing similar patterns?
--   :share-clipboard

plugin = {
    name = "share-to-slack",
    version = "0.1.0",
    description = "Share pane context to Slack or Discord"
}
...
local function build_discord_payload(message)
    return json_encode({
        content = message
    })
end
...
-- Share to Discord
local function share_to_discord(args)
    if not config.discord_webhook then
        enya.notify("error", "DISCORD_WEBHOOK_URL not set. Please set this environment variable.")
        return
    end

    local message = build_context_message(args)
    local payload = build_discord_payload(message)

    local resp = enya.http_post(config.discord_webhook, payload, {
        ["Content-Type"] = "application/json"
    })

    if resp.error then
        enya.notify("error", "Failed to share to Discord: " .. resp.error)
    elseif resp.status >= 400 then
        enya.notify("error", "Discord returned error: " .. (resp.body or "unknown"))
    else
        enya.notify("info", "Shared to Discord!")
    end
end
...
enya.register_command("share-discord", {
    description = "Share current pane context to Discord",
    aliases = { "discord" },
    accepts_args = true
}, share_to_discord)
...
enya.keymap("<leader>sd", "share-discord", "Share to Discord")
enya.keymap("<leader>sy", "share-clipboard", "Copy context to clipboard")

-- Lifecycle hooks
function on_activate()
    local has_slack = config.slack_webhook ~= nil
    local has_discord = config.discord_webhook ~= nil

    if has_slack or has_discord then
        local services = {}
        if has_slack then table.insert(services, "Slack") end
        if has_discord then table.insert(services, "Discord") end
        enya.log("info", "Share plugin ready: " .. table.concat(services, ", "))
    else
        enya.log("warn", "Share plugin: No webhook URLs configured. Set SLACK_WEBHOOK_URL or DISCORD_WEBHOOK_URL.")
    end
end
