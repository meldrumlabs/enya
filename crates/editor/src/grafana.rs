...
use crate::workspace::{
    LayoutConfig, LayoutContainer, LayoutNode, LayoutType, PaneConfig, Workspace,
};
...
impl GrafanaDashboard {
    ...
    pub fn to_workspace(&self) -> Result<ConversionResult, ConversionError> {
        ...
        // Create workspace
        let mut workspace = Workspace::new(&self.title);

        if let Some(desc) = &self.description {
            workspace.workspace.description = desc.clone();
        }
