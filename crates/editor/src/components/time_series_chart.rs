...
impl Series {
    ...
    /// Build a short label for the legend (truncates long names, prefers tag values)
...
impl TimeSeriesChart {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui) {
        ...
        // PlanetScale-style legend above the chart (only show if multiple series)
        if self.show_legend && self.series.len() > 1 {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                // Show "+ N more" if there are overflow series
                if show_overflow {
                    ...
                    if response.hovered() {
                        ...
                        ui.painter().rect_filled(rect, 0.0, palette::bg_surface(self.theme));
                        ui.painter().text(
                            rect.center(),
