...
pub struct QueryPane {
    ...
    is_loading: bool,
    /// Whether edit was requested via button click (for dashboard to pick up)
    edit_requested: bool,
}
...
impl QueryPane {
    ...
    /// Clear the edit requested flag (called after dashboard handles it)
    ...
    pub fn show(&mut self, ui: &mut egui::Ui) -> QueryPaneAction {
        ...
        if !self.buffer_expanded {
            ...
            let button_pos = egui::pos2(pane_rect.right() - button_size.x, pane_rect.top() + 4.0);
