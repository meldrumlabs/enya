...
impl Scanner for JavaScriptPromClientScanner {
    ...
    fn scan_usages(&self, path: &Path) -> Result<Vec<MetricUsage>, ParseError> {
        let mut parser = JavaScriptParser::new()?;
        let (source, tree) = parser.parse_file(path)?;
        let query = parser.create_query(JS_USAGE_QUERY)?;
...
/// Maps JavaScript/prom-client method names to usage kinds.
...
fn scan_usages_tree(
    ...
    while let Some(match_) = matches.next() {
        ...
        let variable_name = extract_js_root_variable(&obj, source);
        if variable_name.is_empty() {
            continue;
        }

        // Extract label values from arguments (for labeled metrics)
        let label_values = extract_js_label_values(&args, source);
...
fn extract_js_root_variable(node: &Node<'_>, source: &str) -> String {
    match node.kind() {
        "identifier" => node
            ...
            if let Some(property) = node.child_by_field_name("property") {
                ...
                if prop_text == "labels" {
                    if let Some(obj) = node.child_by_field_name("object") {
                        return extract_js_root_variable(&obj, source);
                    }
                }
            }
...
fn extract_js_label_values(args: &Node<'_>, source: &str) -> Vec<String> {
...
mod tests {
    ...
    fn parse_and_scan_usages(source: &str) -> Vec<MetricUsage> {
        ...
        let tree = parser.parse(source).expect("Failed to parse");
        let query = parser
            .create_query(JS_USAGE_QUERY)
            .expect("Failed to create query");
        scan_usages_tree(source, &tree, &query, Path::new("test.js")).expect("Failed to scan")
    }
